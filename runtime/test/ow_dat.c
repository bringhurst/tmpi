...
int main(int argc, char *argv[])
{
	...
	if (numprocs!=2) {
		if (myid==io_rank) {
			printf("One-way test requires exactly 2 nodes.\n");
		}
		MPI_Finalize();
		return -1;
	}
...
void master()
{
	...
	for (i=0; i<5; i++) {
		for (j=0; j<18; j++) {
			...
			t2=MPI_Wtime();
			printf("One-way (short), size=%d, iteration=%d, single oneway trip=%fus, rate=%fMB/s.\n", 
					size, ITER, (t2-t1)*MEGA/ITER, (float)size/((t2-t1)*MEGA/ITER));
		}
	}
	...
	for (i=0; i<2; i++) {
		for (j=0; j<18; j++) {
			...
			printf("One-way (long), size=%d, iteration=%d, transfer rate=%fMB/s.\n", 
					size, ITER2, (float)size/((t2-t1)*MEGA/ITER2));
		}
	}
	...
	for (i=2; i<5; i++) {
		for (j=0; j<18; j++) {
			...
			printf("One-way (very long), size=%d, iteration=%d, transfer rate=%fMB/s.\n", 
					size, ITER3, (float)size/((t2-t1)*MEGA/ITER3));
		}
	}
