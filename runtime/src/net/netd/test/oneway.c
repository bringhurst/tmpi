	...
	for (i=0; i<5; i++) {
		for (j=0; j<18; j++) {
			if ((size=short_size[i][j])<0)
				break;
			usc_tmr_reset(&timer);
			if (netd_recvdat(1, (char *)&sig, sizeof(int))<0)
			...
			if (netd_recvdat(1, (char *)&sig, sizeof(int))<0)
				tmpi_error(DBG_INFO, "recvdat failed");
			usc_tmr_stop(&timer);
			printf("One Way (short), size=%d, iteration=%d, average single trip=%fus, rate=%fMB.\n", 
					size, ITER, usc_tmr_read(&timer)/ITER, (float)size/(usc_tmr_read(&timer)/ITER));
			/** sleep(1); */
		}
	}
	...
	for (i=0; i<2; i++) {
		for (j=0; j<18; j++) {
			...
			if (netd_recvdat(1, (char *)&sig, sizeof(int))<0)
				tmpi_error(DBG_INFO, "recvdat failed");
			usc_tmr_stop(&timer);
			printf("One Way (long), size=%d, iteration=%d, transfer rate=%fMB/s.\n", 
					size, ITER2, (float)size/(usc_tmr_read(&timer)/ITER2));
			/* sleep(1); */
		}
	}
	...
	for (i=2; i<5; i++) {
		for (j=0; j<18; j++) {
			...
			if (netd_recvdat(1, (char *)&sig, sizeof(int))<0)
				tmpi_error(DBG_INFO, "recvdat failed");
			usc_tmr_stop(&timer);
			printf("One Way (very long), size=%d, iteration=%d, transfer rate=%fMB/s.\n", 
					size, ITER3, (float)size/(usc_tmr_read(&timer)/ITER3));
			/* sleep(1); */
		}
	}
...
void worker()	
{
	...
	for (i=0; i<5; i++) {
		for (j=0; j<18; j++) {
			if ((size=short_size[i][j])<0)
				break;
			netd_senddat(0, (char *)&start, sizeof(int));
			for (k=0; k<ITER; k++) {
				netd_recvdat(0, msg, size);
			}
			netd_senddat(0, (char *)&end, sizeof(int));
		}
	}
